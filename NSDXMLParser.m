...
//  SmartGitClient
//
//  Created by NullStackDev on 8/30/16.
...
@interface NSDXMLParser () <NSXMLParserDelegate>
...
@implementation NSDXMLParser

+ (NSDXMLParser *)sharedInstance
{
    static dispatch_once_t once;
    static NSDXMLParser *sharedInstance;
    dispatch_once(&once, ^{
        
        sharedInstance = [[NSDXMLParser alloc] init];
    });
...
{
    NSDXMLParser *copy = [[[self class] allocWithZone:zone] init];
...
{
    return [[[NSDXMLParser sharedInstance] copy] dictionaryWithParser:parser];
}

+ (NSDictionary *)dictionaryWithXMLData:(NSData *)data
{
    return [[[NSDXMLParser sharedInstance] copy] dictionaryWithData:data];
}

+ (NSDictionary *)dictionaryWithXMLString:(NSString *)string
{
    return [[[NSDXMLParser sharedInstance] copy] dictionaryWithString:string];
}

+ (NSDictionary *)dictionaryWithXMLFile:(NSString *)path
{
    return [[[NSDXMLParser sharedInstance] copy] dictionaryWithFile:path];
}
...
- (NSString *)innerXML
{
    ...
    {
        [nodes addObject:[NSDXMLParser XMLStringForNode:childNodes[key] withNodeName:key]];
    }
...
- (NSString *)XMLString
{
    ...
    {
        return [NSDXMLParser XMLStringForNode:self withNodeName:[self nodeName] ?: @"root"];
    }
}
